    ...
    try:
        ...
        if process.returncode != 0:
            if "visionias" in cmd and failed_counter <= 10:
                failed_counter += 1
                logging.warning(f"Download failed, attempt {failed_counter}/10")
                await asyncio.sleep(5)
                return await download_video(url, cmd, name)
            else:
                logging.error(f"Download failed: {stderr.decode() if stderr else 'Unknown error'}")
                return None
